...
# On Windows, a pre-compiled executable can be downloaded automatically
# with download_swat_exe().
# On Linux and macOS, users must compile the SWAT source code for their
# system and provide the executable path.
# Source code is available at:
# https://swat.tamu.edu/software/swat/
...
#
# Define the objective functions, target variables, and time periods
# used to evaluate model performance during calibration.
...
# Prepare observed data in the format required by hydroSWAT for
# calibration
observed_data <- list(
  daily = list(
    rch = list(
...
# This ensures hydroSWAT can compare the simulated output with the
# observed data defined above.
output_config <- list(
  rch = list(
    file              = "output.rch",
...
# calibration method (e.g., absolute value or relative change),
# acceptable ranges, and target model components.
...
# evaluations). On a Dell XPS 15 9510 laptop (Intel Core i9-11900H,
# 8 cores, 2.50 GHz, 64 GB RAM), this configuration required
...
#
# Extract the best compromise solution from the Pareto front and
# visualize the performance trade-offs across objectives.

best_solution <- best_compromise_solution(
  calibration_result$objectives
)

best_parameters <- calibration_result$parameters[
  best_solution$index, ]

parameter_info$value <- best_parameters

best_objectives <- calibration_result$objectives[
  best_solution$index, ]
...
#
# Run the SWAT model once using the best-performing parameter set
# obtained from the multi-objective calibration.
...
# the Nash–Sutcliffe Efficiency (NSE), Kling-Gupta Efficiency (KGE),
# Mean Absolute Error (MAE), and Percent Bias (PBIAS), and are
# complemented with diagnostic plots, such as hydrographs, flow
# duration curves, and seasonal cycles, for comprehensive model
# evaluation.
...
plot_mean_annual_cycle(
  evaluation_results$mean_annual_cycle
)
...
sub_data <- subs1 %>%
  ...
  select(Subbasin, PRECIPmm, ETmm, WYLDmm) %>%
  pivot_longer(
...
# 12. Final notes
#
# The SWAT executable is downloaded using download_swat_exe(), but users
# may also provide the path to their preferred version.
#
