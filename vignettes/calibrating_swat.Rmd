---
title: "Calibrating SWAT with hydroSWAT: A complete workflow"
...
vignette: >
  ...
  %\VignetteIndexEntry{Calibrating SWAT with hydroSWAT: A complete workflow}
...
This vignette presents a complete and reproducible workflow for calibrating a 
SWAT model using the `hydroSWAT` R package. The example dataset corresponds
to the Huancane River Basin in southern Peru, modeled using QSWAT.
...
> Due to external dependencies, parallel execution, and computational cost,
> this vignette is not run automatically during package checks and is intended
...
> - On **Windows**, a pre‑compiled executable can be downloaded automatically with `download_swat_exe()`.  
> - On **Linux** and **macOS**, users must **compile the SWAT source code** for their system and provide the executable path.  
> Source code is available at [https://swat.tamu.edu/software/swat/](https://swat.tamu.edu/software/swat/).  
...
-   The SWAT executable is downloaded using `download_swat_exe()`, but
    users may also provide the path to their preferred version.
